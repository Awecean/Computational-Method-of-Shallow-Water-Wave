% This program aims to read simulation results caused by the different
% initial condition. And Find a propable initial field by solving coefficient matrix.
clear;
close all;
...
for i = 1:7
    ...
    for j = 1:61 % fit the data t = 500 to 3500 every 50s 
        eta_station(i+7*(j-1)) = interp1(x,y,500+(j-1)*50,'linear');
    end
...
ICresult = zeros(68,28);
count = 1;
for i = 1:3
    for j = 1:4
        load(['newdata\simulated_' sprintf('%d_%d.mat',i,j)]);
        for k = 1:61
            temp = cellresult(:,:,1*(k-1)+11); % the field value of each moment
...
save('PI.mat','PI'); % saving the propable intial condition.
