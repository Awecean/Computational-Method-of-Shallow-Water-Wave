function [eta_next, U_next, H_next] = ssprkB(etanow, Unow, h, hp, hm, dt, dx, nL)
%Instruction
...
% eta_next, U_next, H_next
    g = 9.81;
    [H_temp1, H_temp2, H_next, HU_temp1, HU_temp2, HU_next] = deal(zeros(1,nL));
    H = etanow+h;

    % round 1-phase 1 impose bodary condition and the muscl, hllc
    etanow = [etanow(4) etanow(3) etanow(3:end-2) etanow(end-2) etanow(end-3)];
    U = [-Unow(4) -Unow(3) Unow(3:end-2) -Unow(end-2) -Unow(end-3)];
    HU = H.*U;
    [Um, Up] = wv.muscl(U); [etam, etap] = wv.muscl(etanow);
    [F, G] = wv.hllc(etam, etap, Um, Up, hm, hp, h);
    % round 1-phase 2 SSP-RK
    for i = 3:nL-2
        H_temp1(i) = H(i)-dt/dx*(F(i+1)-F(i));
        HU_temp1(i) = HU(i)-dt/dx*(G(i+1)-G(i))+dt/2/dx*g*etanow(i)*(h(i+1)-h(i-1));
    end
    %fprintf(min(abs(H_temp1)));
    eta_temp1 = H_temp1-h;
    U_temp1 = HU_temp1./H_temp1;
    % round 2-phase 1 
